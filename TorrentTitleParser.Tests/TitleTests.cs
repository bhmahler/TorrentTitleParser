...
namespace TorrentTitleParser.Tests
{
    ...
    public class TitleTests
    {
        ...
        public void TestMovieTitles()
        {
            ...
            torrent = new Torrent("Inside Out 2 2024 1080p TELESYNC x264 COLLECTiVE");
            Assert.AreEqual("Inside Out 2", torrent.Title);
            Assert.AreEqual(2024, torrent.Year);
            Assert.AreEqual("1080p", torrent.Resolution);
            Assert.AreEqual("TELESYNC", torrent.Quality);
            Assert.AreEqual("x264", torrent.Codec);
            Assert.AreEqual("COLLECTiVE", torrent.Group);
            Assert.IsTrue(torrent.IsPreRelease);
